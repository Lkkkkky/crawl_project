...
def get_timestamp():
    ...
    return timestamp
def send_request(round:str,keyword:str,tk,ip):  #发送请求包
    ...
    try:
        response = requests.post('https://mxsa.mxbc.net/api/v1/h5/marketing/secretword/confirm', proxies=proxy,headers=headers, params=params,data=N.encode(), timeout=1)
...
def get_proxy():
    ...
    r = requests.get("http://pandavip.xiongmaodaili.com/xiongmao-web/apiPlus/vgb?secret=b6840f11b6ca4cb1bf1e6e022eea51f9&orderNo=VGB20240811173429K5tVJgOm&count=1&isTxt=0&proxyType=1&validTime=0&removal=0&cityIds=&returnAccount=2", verify=False, allow_redirects=False)
    r.encoding='utf8'
...
def getit(round:str,secretword:str,tk):
    ...
    while check==False:
        ...
        for ip_dict in proxy_dict:
            ...
            pro[i]=ip
            print(pro[0])
            check=test_proxy(ip,'https://2024.ip138.com')
    s = send_request(round, secretword,tk,pro[i])
...
if __name__ == '__main__':
    ...
    t1 = threading.Thread(target=getit,args=(round,keyword,tk1,))
